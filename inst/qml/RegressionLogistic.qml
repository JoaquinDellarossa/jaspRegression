...
Form
{
	...
	VariablesForm
	{
		...
		AssignedVariablesList { name: "weights";	title: qsTr("WLS Weights (optional)"); info: qsTr("The weights used for weighted least square regression.") ;allowedColumns: ["scale"]; 	singleVariable: true; debug: true	}
	}
	...
	Section
	{
		...
		Group
		{
			...
			CheckBox { name: "coefficientEstimate";	label: qsTr("Estimates"); info: qsTr("Coefficient estimates, standard errors, z-values, and their corresponding p-values.") ; checked: true; id: coefficientEstimate
                ...
                {
                    id: bootstrapping
                    name: "coefficientBootstrap"; label: qsTr("From"); info: qsTr("By selecting this option, bootstrapped estimation is applied. By default, the number of replications is set to 1000. This can be changed into the desired number.")
            ...
            CheckBox { name: "multicollinearity"; label: qsTr("Multicollinearity diagnostics"); info: qsTr("Display Tolerance and Variance Inflation Factor for each predictor in the model to assess multicollinearity.") }
		}        
        ...
        Group
        {
			...
			CheckBox { name: "fMeasure";		label: qsTr("F-measure"); info: qsTr("This is based on the amount of systematic variance divided by the amount of unsystematic variance i.e., mean squares for the model / the residual mean squares")			}
	...
	Section
	{
		...
		Group
		{
			title: qsTr("Diagnostic Plots"); info: qsTr("If the assumptions of the linear regression model are tenable, then these residuals should scatter randomly about a horizontal line. Any systematic pattern or clustering of the residuals suggests a model violation/s.")
			...
			{ 
				...
				CheckBox { name: "independentVsPredictedPlotIncludeInteractions";	label: qsTr("Include interactions"); info: qsTr("also add every two-way interaction") ;checked: true}
				CheckBox { name: "independentVsPredictedPlotUseLogit";	label: qsTr("Use logit scale"); info: qsTr("plot predicted probabilities on the logit scale, to ensure a linear relation") ;checked: true}
		...
		RadioButtonGroup
		{
			...
			RadioButton { value: "deviance";	label: qsTr("Deviance"); info: qsTr("The standardized deviance rwsidual") ;	checked: true   }
